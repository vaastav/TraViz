...
class Overview extends Component {
    ...
    createOverview() {
        ...
        var axis = d3.axisBottom().ticks(10).tickSize(6, 0, 0);
        ...
        function overview(selection) {
            selection.each(function (layout) {
                ...
                var mini = d3.select(node).selectAll(".mini").data([layout]);
                ...
                mini.select(".hitarea").attr('width', width).attr('height', height)
                    ...
                    .on('mouseup', function () {
                        var point = sx.invert(d3.mouse(node)[0]);
        ...
        overview.refresh = function (selection) {
            selection.each(function (layout) {
                d3.select(node).selectAll(".mini").data([layout]).select(".brush").call(brush);
            });
        };
