    ...
    createControlPanelButton() {
        ...
        function button(selection, layout) {
          selection.each(function(d, i) {
            ...
            d3.select(node).select("text").text(d.enabled ? d.lenabled : d.ldisabled).attr("dominant-baseline", "middle")
    ...
    createControlPanel() {
        ...
        function controlpanel(selection) {
            selection.each(function (layout) {
                ...
                var cp = d3.select(node).selectAll(".controlpanel").data([layout]);
        ...
        controlpanel.refresh = function (selection) {
            selection.each(function (layout) {
                ...
                bs.on("click", function (d) { d.enabled = !d.enabled; callbacks["property"].call(node, d.property, d.enabled); });
            });
        };
